...
public class HeapPage {
	...
	public int getNumSlots() {
		//your code here
		int slots = Math.floorDiv(HeapFile.PAGE_SIZE*8,td.getSize()*8+1);
		return slots;
	}
	...
	public int getHeaderSize() {        
		//your code here
		int a = (int) Math.ceil(getNumSlots()/8.0);
...
//		//your code here
	...
	public void setSlotOccupied(int s, boolean value) {
		...
		int bitPos = s%8;
		if(value) {
			header[bytePos] = (byte) (header[bytePos]|(1<<bitPos));
		}
		else {
			header[bytePos] = (byte)(header[bytePos]& ~(1<<bitPos));
		}
	...
	public void addTuple(Tuple t) throws Exception {
		//your code here
		//go over the slot to see if occupied
	...
	public void deleteTuple(Tuple t) throws Exception {
		...
		for(int i = 0;i<getNumSlots();i++) {
			if(slotOccupied(i)) {
				Tuple temp = tuples[i];
				if(temp.getDesc()==t.getDesc()&&temp.getPid()==t.getPid()) {
//					t.setId((Integer)null);
